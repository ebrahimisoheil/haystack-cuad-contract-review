...
def _qa(title: str, context: str, category: str, answer: str | None) -> dict:
    answers = [] if answer is None else [{"text": answer, "answer_start": context.index(answer)}]
    return {
                ...
                "paragraphs": [
                    {
                        "context": context,
                        "qas": [
                            _qa(title, context, "Governing Law", "Delaware" if "Delaware" in context else None),
                            _qa(title, context, "Document Name", "VENDOR SAAS AGREEMENT" if "VENDOR SAAS AGREEMENT" in context else None),
                        ],
                    }
                ],
            }
...
def test_ingestion_is_bounded_reproducible_and_span_preserving(tmp_path: Path) -> None:
    ...
    assert [item.title for item in first.contracts] == [item.title for item in second.contracts]
